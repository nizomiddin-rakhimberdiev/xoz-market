...
function validateOrderInput(input: unknown): { valid: boolean; errors: ValidationError[]; data?: OrderInput } {
  ...
  // Validate store_id
  if (typeof data.store_id !== 'string' || !validateUUID(data.store_id)) {
    errors.push({ field: 'store_id', message: 'Valid store ID is required' });
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: store, error: storeError } = await supabase
      .from('stores')
      .select('id, name, slug, is_active, store_status, telegram_bot_token, telegram_chat_id')
    ...
    if (!store.is_active || store.store_status !== 'active') {
      return new Response(
        JSON.stringify({ error: 'Store is not active' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    for (const item of input.items) {
      if (item.variant_id) {
        ...
        await supabase.rpc('decrement_variant_stock', {
          ...
          if (error) console.error('Variant stock decrement error:', error);
        });
      }
      // Product stock (always decrement main product)
      await supabase.rpc('decrement_product_stock', {
        p_product_id: item.product_id,
        p_qty: item.quantity,
      }).then(({ error }) => {
        if (error) console.error('Product stock decrement error:', error);
      });
    }

    // Send Telegram notification using store's bot credentials (non-blocking)
    try {
      const TELEGRAM_BOT_TOKEN = store.telegram_bot_token || Deno.env.get('TELEGRAM_BOT_TOKEN');
      const TELEGRAM_CHAT_ID = store.telegram_chat_id || Deno.env.get('TELEGRAM_CHAT_ID');
...
${getDeliveryTypeLabel(order.delivery_type)}${order.delivery_address_text ? `\n📍 <b>Manzil:</b> ${order.delivery_address_text}` : ''}
...
💰 <b>Jami:</b> ${formatPrice(order.total_amount)}${order.comment ? `\n\n💬 <b>Izoh:</b> ${order.comment}` : ''}`;
